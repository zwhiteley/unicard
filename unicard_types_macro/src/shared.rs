    ...
    let init = fields
        ...
        .map(|field| {
            ...
            quote_spanned!(ty.span() =>
                <#ty as ::unicard_types::WasmType>::read(&mut *reader)?
            )
        });
...
pub fn read_normal_ctor(fields: &FieldsNamed) -> TokenStream {
    let field_init = fields
        ...
        .iter()
        .map(|field| {
            let name = field.ident.as_ref().expect("fields on a named struct must be named");
            let ty = &field.ty;

            quote_spanned!(ty.span() =>
                #name: <#ty as ::unicard_types::WasmType>::read(&mut *reader)?
            )
        });
